...
static const QString MENUBAR_STYLE = R"(
    QMenuBar {
        background: #050d1a;
        color: #a0aec0;
        border-bottom: 1px solid #1a2a40;
...
void MainWindow::setupMenuBar()
{
    ...
    bar->setStyleSheet(MENUBAR_STYLE);

    // Help menu
    QMenu *helpMenu = bar->addMenu("Help");

    QAction *updateAction = new QAction("Check for Database Updates", this);
    updateAction->setStatusTip("Check if a newer material database is available");
    connect(updateAction, &QAction::triggered, this, &MainWindow::onCheckForUpdates);

    QAction *aboutAction = new QAction("About Nadir", this);
    connect(aboutAction, &QAction::triggered, this, [this](){
        QMessageBox box(this);
        box.setWindowTitle("About Nadir");
        box.setStyleSheet(DIALOG_STYLE);
        box.setText(
            "<span style='color:#6F7CFF; font-size:16px; font-weight:bold;'>NADIR</span>"
            "<br><span style='color:#4FD1C5;'>Material Selection Engine</span>"
            "<br><br><span style='color:#cbd5e0;'>Version 1.0.0</span>"
            "<br><span style='color:#cbd5e0;'>Developed by Zyncrate</span>"
            "<br><br><span style='color:#a0aec0; font-size:11px;'>"
            "Built on Qt 6.10.1 &bull; SQLite &bull; C++17</span>"
            );
        box.exec();
    });
...
// ── DB VERSION ───────────────────────────────────────────────
int MainWindow::getLocalDbVersion()
{
    QString dbPath = QCoreApplication::applicationDirPath() + "/materials.db";
    sqlite3 *db = nullptr;
...
void MainWindow::onCheckForUpdates()
{
    ...
    if (localVersion < 0) {
        QMessageBox::warning(this, "Error",
                             "Could not read local database version.");
        return;
    }

    // fetch version.txt from GitHub releases
    QUrl url("https://github.com/Zyncrate/Nadir-GUI/releases/latest/download/version.txt");
    ...
    QMessageBox::information(this, "Checking for Updates",
                             "Checking for database updates...\nYou will be notified of the result.");
}

void MainWindow::onVersionFetched(QNetworkReply *reply)
{
    ...
    bool ok = false;
    remoteVersion = data.toInt(&ok);

    if (!ok || remoteVersion < 1) {
        QMessageBox::warning(this, "Update Check Failed",
                             "Received an invalid version response from the server.");
        return;
    }
    ...
    if (remoteVersion <= localVersion) {
        QMessageBox::information(this, "Up to Date",
                                 QString("Your database is up to date.\n\nLocal version: %1\nServer version: %2")
                                     .arg(localVersion).arg(remoteVersion));
        return;
    }

    // newer version available — ask user
    int choice = QMessageBox::question(this, "Update Available",
                                       QString("A new database is available!\n\n"
                                               "Your version: %1\nNew version: %2\n\n"
    ...
    // download the new db
    QUrl dbUrl("https://github.com/Zyncrate/Nadir-GUI"
               "/releases/latest/download/materials.db");
    QNetworkRequest dbRequest(dbUrl);
    dbRequest.setAttribute(QNetworkRequest::RedirectPolicyAttribute,
                           QNetworkRequest::NoLessSafeRedirectPolicy);

    QNetworkReply *dbReply = networkManager->get(dbRequest);
    connect(dbReply, &QNetworkReply::finished, this, [this, dbReply](){
        onDatabaseDownloaded(dbReply);
        dbReply->deleteLater();
    });
}

void MainWindow::onDatabaseDownloaded(QNetworkReply *reply)
{
    if (reply->error() != QNetworkReply::NoError) {
        QMessageBox::warning(this, "Download Failed",
                             "Could not download the new database.\n\n"
                             "Error: " + reply->errorString());
        return;
    }
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Write Failed",
                             "Could not write the new database to disk.\n"
    ...
    // reload materials
    allMaterials.clear();
    Metals.clear(); Ceramic.clear(); Polymer.clear();
    Composite.clear(); Semiconductor.clear(); Natural.clear();
    ...
    QMessageBox::information(this, "Update Complete",
                             QString("Database updated successfully to version %1.\n"
                                     "%2 materials now loaded.")
                                 .arg(remoteVersion)
                                 .arg(allMaterials.size()));
}
...
void MainWindow::showMaterialDetail(const Material &m)
{
                        ...
                        const QList<QPair<QString,QString>> &rows)
    {
        ...
        for (const auto &r : rows) {
            ...
            key->setStyleSheet("color:#cbd5e0;");
